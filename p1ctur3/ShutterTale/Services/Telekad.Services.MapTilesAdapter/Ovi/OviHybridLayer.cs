...
{
    public class OviHybridLayer : OviBaseMapLayer
    {
        ...
        public OviHybridLayer(IConfigurationElement configuration)
            : base(configuration)
        {
            this.urlFormat = string.IsNullOrEmpty(configuration["url"])
                                 ? "http://{3}.maptile.maps.svc.ovi.com/maptiler/v2/maptile/newest/hybrid.day/{0}/{1}/{2}/256/png"
                                 : configuration["url"];
        }
