...
{
    public class GoogleServerVectorRasterLayer : GoogleServerBaseMapLayer
    {
        ...
        public GoogleServerVectorRasterLayer(IConfigurationElement configuration)
            : base(configuration)
        {
            var urlFormatConfig = string.IsNullOrEmpty(configuration["url"])
                                 ? "http://{5}/{6}/query?request=VectorMapsRaster&channel={3}&version={4}&x={0}&y={1}&z={2}"
                                 : configuration["url"];
